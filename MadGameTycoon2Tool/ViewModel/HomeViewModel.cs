...
namespace MadGameTycoon2Tool.ViewModel
{
    internal class HomeViewModel : ObservableObject
    {
        ...
        public void MainType_SelectionChanged()
        {
            ...
            foreach (var item in jo[Home.SelectMainType]["GameTheme"])
            {
                gameThemeList.Add(item.ToString());
            }
            Home.Theme = GameTypeAndTheme.GetGameTheme(gameThemeList);
            AppConfig.GameThemeTemp = Home.Theme;

            //根据主类型获得合适的子类型
            Home.SelectChildType = string.Empty;
            Home.ChildType = GameTypeAndTheme.GetChildType(jo, Home.SelectMainType);

            //根据单个主类型获取设计重点、设计方向、开发重点
            GameDesign.GetSingleType_DesignFocus(jo, Home);
            GameDesign.GetSingleType_DesignDirection(jo, Home);
            GameDesign.GetType_DesignPriority(jo, Home);
        }
        /// <summary>
        /// 子类型combobox发生改变以后
        /// </summary>
        public void ChildType_SelectionChanged()
        {
            ...
            //然后会跳到这个事件，如果不处理会报错
            if (Home.SelectChildType == "")
            {
                return;
            }
            JObject jo = JsonConvert.DeserializeObject(File.ReadAllText(AppConfig.GameInfoJson)) as JObject;
            JObject jo2 = JsonConvert.DeserializeObject(File.ReadAllText(AppConfig.GameInfo2Json)) as JObject;
            //根据相同类型获得合适的主题
            List<string> sameTheme = GameTypeAndTheme.GetSameGameTheme(jo, Home.SelectMainType, Home.SelectChildType);
            Home.Theme = GameTypeAndTheme.GetGameTheme(sameTheme);
            AppConfig.GameThemeTemp = Home.Theme;

            //根据主类型和子类型获取设计重点、设计方向、开发重点
            GameDesign.GetMultipleype_DesignFocus(jo2, Home);
            GameDesign.GetMultipleype_DesignDirection(jo2, Home);
            GameDesign.GetType_DesignPriority(jo, Home);
        }
        /// <summary>
        /// 查找游戏主题
        ...
        {
            Home.Theme = AppConfig.GameThemeTemp;//先把界面上显示的主题设置成全部合适的主题
            List<string> searchResult = new List<string>();
            string searchText = Home.SearchTheme;//要搜索的主题
            if (!string.IsNullOrWhiteSpace(searchText))//如果不是null或者空格再搜索
            {
                foreach (var item in Home.Theme)//从所有合适的主题里查找
                {
                ...
                Home.Theme = searchResult;
            }
            else
            {
                Home.Theme = AppConfig.GameThemeTemp;
            }
        }
    }
}
